...
const API_URL =
  process.env.GEOSCORE_API_URL || "https://geoscore.sucana.ai";
...
server.tool(
  "analyze_url",
  "Analyze a webpage for AI search visibility (GEO — Generative Engine Optimization). Returns a score out of 100 across 10 categories with actionable fixes for ChatGPT, Claude, Gemini, and Perplexity discoverability.",
  {
  ...
  async ({ url }) => {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Error: ${(error as { error?: string }).error || `HTTP ${response.status}`}`,
            },
          ],
        };
      }
      ...
      const categories = (
        ...
        .map(
          (c) =>
            `  ${c.name}: ${c.score}/10 (weight: ${c.weight}%) — ${c.findings.filter((f) => f.type === "critical" || f.type === "warning").length} issues`
        )
        .join("\n");
      ...
      const recs = (
        result.generatedFixes.contentRecommendations as string[]
      )
        .map((r) => `- ${r}`)
        .join("\n");
